...
import { buildConsoleRoute } from "../utils/routePaths";
...
export function BriefingExportPanel({ scenarioLabel, result, point, currentView, onExport }: BriefingExportPanelProps) {
    ...
    () => {
      return {
        executive: {
          ...
          description: "Open the executive brief surface before downloading the styled PDF.",
          routePath: "/export/executive-brief",
        },
        presentation: {
          artifact: "presentation",
          title: "Presentation Brief",
          description: "Open the slide-based layout before downloading the presentation PDF.",
          routePath: "/export/presentation-brief",
        },
        board: {
          artifact: "board",
          title: "Board One Pager",
          description: "Inspect the board one-pager in-console before downloading the PDF.",
  ...
  const openPreview = (artifact: PreviewArtifactKey) => {
    ...
    if (artifact === "executive" && !normalized.availabilityByMode["executive-brief"].exportable) {
      window.alert(normalized.availabilityByMode["executive-brief"].reason ?? "Executive Brief withheld pending structural state integrity.");
      return;
    }
    ...
    try {
      ...
      const previewRoute = buildConsoleRoute(window.location.pathname, preview.routePath);
      openPreviewRoute(`${previewRoute}?token=${encodeURIComponent(token)}`);
    } catch (error) {
      ...
      JSON.stringify(
        {
          ...
          briefingStateV2: briefingState.v2,
          simulationClarity: {
            replay: "Observed deterministic replay of visible artifacts through the bounded-world engine.",
            projection: "Synthetic forward artifacts generated by the projection engine and kept separate from replay.",
            counterfactual: "Sandbox mutation of artifact timing, weight, or presence for bounded sensitivity testing.",
            preGcsSensitivity:
              "Diagnostic response layer derived from current state and artifact distribution; not projection, prediction, or recommendation.",
          },
        },
        null,
        2,
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-muted">
            Prepare export artifacts for review, audit handoff, or a point-in-time replay record.
          </p>
        </div>
      ...
      <div className="mt-4 grid gap-3">
        <div className="surface-panel-subtle flex items-center justify-between gap-4 p-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">Styled executive brief for circulation or direct review.</p>
          </div>
        ...
        <div className="surface-panel-subtle flex items-center justify-between gap-4 p-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">Slide-based layout for meeting prep, review, or print.</p>
          </div>
        ...
        <div className="surface-panel-subtle flex items-center justify-between gap-4 p-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">Concise summary sheet for board, committee, or post-meeting follow-up.</p>
          </div>
        ...
        <div className="surface-panel-subtle flex items-center justify-between gap-4 p-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">Transitions and proof objects prepared for governance or challenge review.</p>
          </div>
