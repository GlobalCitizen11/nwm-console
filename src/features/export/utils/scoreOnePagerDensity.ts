...
export const scoreSignalDensity = (modules: ExportModulePayload[]) =>
  Math.max(0, 100 - Math.abs(18 - modules.reduce((sum, module) => sum + module.estimatedHeight, 0)) * 6);

export const scoreGridBalance = (modules: ExportModulePayload[]) => (modules.length >= 4 ? 100 : 72);
